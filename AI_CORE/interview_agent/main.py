...
from interview_agent.utils import handle_interview_chat, get_session_state, clear_user_session
import uvicorn
...
async def interview_chat(req: InterviewRequest):
    ...
    if not req.user_input or not req.user_input.strip():
        raise HTTPException(
            status_code=400, detail="user_input cannot be empty")
...
async def get_session(user_id: str):
    ...
    if state is None:
        raise HTTPException(
            status_code=404, detail="Session not found for user")
